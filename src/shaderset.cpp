...
static uint64_t GetShaderFileTimestamp(const char *filename) {
...
#ifdef _WIN32
    int filenameBufferSize = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS,
                                                 filename, -1, NULL, 0);
    ...
    if (MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, filename, -1,
                            wfilename, filenameBufferSize)) {
        // Potential improvement: How can we poll the timestamp without
        // sometimes blocking write access from other programs? (OS race
        // condition)
        HANDLE hFile = CreateFileW(wfilename, GENERIC_READ,
...
ShaderSet::~ShaderSet() {
    ...
    for (std::pair<const std::vector<const ShaderNameTypePair *>, Program>
             &program : mPrograms) {
        glDeleteProgram(program.second.InternalHandle);
    }
}

void ShaderSet::SetVersion(const std::string &version) { mVersion = version; }
...
GLuint *ShaderSet::AddProgram(
    const std::vector<std::pair<std::string, GLenum>> &typedShaders) {
    std::vector<const ShaderNameTypePair *> shaderNameTypes;

    // find references to existing shaders, and create ones that didn't exist
    // previously.
    for (const std::pair<std::string, GLenum> &shaderNameType : typedShaders) {
        ShaderNameTypePair tmpShaderNameType;
        std::tie(tmpShaderNameType.Name, tmpShaderNameType.Type) =
            shaderNameType;
    ...
    shaderNameTypes.erase(
        std::unique(begin(shaderNameTypes), end(shaderNameTypes)),
        end(shaderNameTypes));
    ...
    if (!foundProgram->second.InternalHandle) {
        ...
        for (const ShaderNameTypePair *shader : shaderNameTypes) {
            glAttachShader(foundProgram->second.InternalHandle,
...
void ShaderSet::UpdatePrograms() {
    ...
    for (std::pair<const ShaderNameTypePair, Shader> *shader : updatedShaders) {
        // the #line prefix ensures error messages have the right line number
        // for their file
        // the #line directive also allows specifying a "file name" number,
        ...
        const char *strings[] = {version.c_str(), premable.c_str(),
                                 source.c_str()};
        GLint lengths[] = {(GLint)version.length(), (GLint)premable.length(),
                           (GLint)source.length()};

        glShaderSource(shader->second.Handle,
                       sizeof(strings) / sizeof(*strings), strings, lengths);
        ...
        if (!status) {
            GLint logLength;
            glGetShaderiv(shader->second.Handle, GL_INFO_LOG_LENGTH,
                          &logLength);
            std::vector<char> log(logLength + 1);
            glGetShaderInfoLog(shader->second.Handle, logLength, NULL,
                               log.data());

            std::string log_s = log.data();

            // replace all filename hashes in the error messages with actual
            // filenames
            for (size_t found_preamble;
                 (found_preamble = log_s.find(preamble_hash)) !=
                 std::string::npos;) {
                log_s.replace(found_preamble, preamble_hash.size(), "preamble");
            }
            for (size_t found_source;
                 (found_source = log_s.find(source_hash)) !=
                 std::string::npos;) {
                log_s.replace(found_source, source_hash.size(),
                              shader->first.Name);
            }
    ...
    // relink all programs that had their shaders updated and have all their
    // shaders compiling successfully
    for (std::pair<const std::vector<const ShaderNameTypePair *>, Program>
             &program : mPrograms) {
        bool programNeedsRelink = false;
        for (const ShaderNameTypePair *programShader : program.first) {
            for (std::pair<const ShaderNameTypePair, Shader> *shader :
                 updatedShaders) {
        ...
        if (programNeedsRelink) {
            for (const ShaderNameTypePair *programShader : program.first) {
                GLint status;
                glGetShaderiv(mShaders[*programShader].Handle,
        ...
        if (programNeedsRelink && canRelink) {
            ...
            GLint logLength;
            glGetProgramiv(program.second.InternalHandle, GL_INFO_LOG_LENGTH,
                           &logLength);
            std::vector<char> log(logLength + 1);
            glGetProgramInfoLog(program.second.InternalHandle, logLength, NULL,
                                log.data());
            ...
            for (const ShaderNameTypePair *shaderInProgram : program.first) {
                std::string source_hash =
                    std::to_string(mShaders[*shaderInProgram].HashName);
                for (size_t found_source;
                     (found_source = log_s.find(source_hash)) !=
                     std::string::npos;) {
                    log_s.replace(found_source, source_hash.size(),
                                  shaderInProgram->Name);
                }
            }

            GLint status;
            glGetProgramiv(program.second.InternalHandle, GL_LINK_STATUS,
                           &status);
