...
class ShaderSet {
    ...
    struct Shader {
        ...
        uint64_t Timestamp;
        // Hash of the name of the shader. This is used to recover the shader
        // name from the GLSL compiler error messages.
    ...
    struct Program {
        // The handle exposed externally ("public") and the most recent
        // (succeeding/failed) linked program ("internal")
        // the public handle becomes 0 when a linking failure happens, until the
        // linking error gets fixed.
    ...
    std::string mVersion;
    // the preamble which gets prepended to each shader (for eg. shared binding
    // conventions)
  ...
  public:
    ...
    // eg: AddProgram({ {"foo.vert", GL_VERTEX_SHADER}, {"bar.frag",
    // GL_FRAGMENT_SHADER} });
    // To be const-correct, this should maybe return "const GLuint*". I'm
    // trusting you not to write to that pointer.
    GLuint *
    AddProgram(const std::vector<std::pair<std::string, GLenum>> &typedShaders);

    // Polls the timestamps of all the shaders and recompiles/relinks them if
    // they changed
    void UpdatePrograms();
