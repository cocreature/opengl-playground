...
// self-packing freelist implementation based on
...
template <class T> class packed_freelist {
    ...
    struct allocation_t {
        ...
        // * The 16 LSBs store the index of this allocation in the list of
        // allocations
        // * The 16 MSBs store the number of times this allocation struct was
        // used to allocate an object
        //      * this is used as a (non-perfect) counter-measure to reusing IDs
        //      for objects
        uint32_t allocation_id;

        // the index in the objects array which stores the allocated object for
        // this allocation
        uint16_t object_index;

        // the index in the allocations array for the next allocation to
        // allocate after this one
        uint16_t next_allocation;
    };

    // Storage for objects
    // Objects are contiguous, and always packed to the start of the storage.
    // Objects can be relocated in this storage thanks to the separate list of
    // allocations.
    std::size_t _num_objects;
    std::size_t _max_objects;
    std::size_t _cap_objects;
    T *_objects;
    ...
    // when an allocation is freed, the enqueue index struct's next will point
    // to it
    ...
    // note objects are still not guaranteed to have globally unique IDs, since
    // IDs will be reused after N * 2^16 allocations
  ...
  public:
    struct iterator {
        iterator(uint32_t *in) { _curr_object_alloc_id = in; }
        ...
        uint32_t operator*() { return *_curr_object_alloc_id; }
    ...
    packed_freelist(std::size_t max_objects) {
    ...
    ~packed_freelist() {
        for (std::size_t i = 0; i < _num_objects; i++) {
            _objects[i].~T();
        }
    ...
    packed_freelist(const packed_freelist &other) {
        ...
        for (std::size_t i = 0; i < other._num_objects; i++) {
            new (_objects + i) T(*(other._objects + i));
            _object_alloc_ids[i] = other._object_alloc_ids[i];
        }

        for (std::size_t i = 0; i < other._max_objects; i++) {
    ...
    packed_freelist(packed_freelist &&other) : packed_freelist() {
        swap(other);
    }
    ...
    bool contains(uint32_t id) const {
        // grab the allocation by grabbing the allocation index from the id's
        // LSBs
        allocation_t *alloc = &_allocations[id & alloc_index_mask];

        // * NON-conservative test that the IDs match (if the allocation has
        // been reused 2^16 times, it'll loop over)
        // * Also check that the object is hasn't already been deallocated.
        //      * This'll prevent an object that was just freed from appearing
        //      to be contained, but still doesn't disambiguate between two
        //      objects with the same ID (see first bullet point)
    ...
    void erase(uint32_t id) {
        ...
        // if necessary, move (aka swap) the last object into the location of
        // the object to erase, then unconditionally delete the last object
        if (alloc->object_index != _num_objects - 1) {
            T *last = _objects + _num_objects - 1;
            *o = std::move(*last);
            o = last;

            // since the last object was moved into the deleted location, the
            // associated object ID array's value must also be moved similarly
            _object_alloc_ids[alloc->object_index] =
                _object_alloc_ids[_num_objects - 1];

            // since the last object has changed location, its allocation needs
            // to be updated to the new location.
            _allocations[_object_alloc_ids[alloc->object_index] &
                         alloc_index_mask]
                .object_index = alloc->object_index;
        }
        ...
        _allocations[_last_allocation].next_allocation =
            alloc->allocation_id & alloc_index_mask;
    ...
    iterator begin() const { return iterator{_object_alloc_ids}; }

    iterator end() const { return iterator{_object_alloc_ids + _num_objects}; }

    bool empty() const { return _num_objects == 0; }

    std::size_t size() const { return _num_objects; }

    std::size_t capacity() const { return _max_objects; }
  ...
  private:
    allocation_t *insert_alloc() {
        ...
        // increment the allocation count in the 16 MSBs without modifying the
        // allocation's index (in the 16 LSBs)
...
template <class T> void swap(packed_freelist<T> &a, packed_freelist<T> &b) {
    a.swap(b);
}
